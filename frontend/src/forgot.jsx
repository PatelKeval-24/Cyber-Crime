...
function ForgotPassword() {
  ...
  const handleSendOtp = async () => {
    ...
    setEmailError("");
    setLoading(true);
    await axios.post(`${import.meta.env.VITE_BACKEND_URL}/auth/forgot-password`, { email,latitude,longitude });
    setLoading(false);
    setStep(2);
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/auth/verify-otp`, { email, otp,latitude,longitude });
