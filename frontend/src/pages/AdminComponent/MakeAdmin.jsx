...
// ── Status badge ─────────────────────────────────────────────────────────────
const StatusBadge = ({ status, onApprove, elem }) => {
  ...
  return (
    <div 
      ...
      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full shrink-0 cursor-pointer ${cls} hover:bg-emerald-400/20 transition-colors`}
    >
      <button className="text-xs font-mono font-medium ">{"Make Admin"}</button>
    </div>
  )
}
...
// ── Volunteer card ────────────────────────────────────────────────────────────
const VolunteerCard = ({ elem, index, onMakeAdmin }) => {
...
const Toast = ({ msg, type }) => (
  <div style={{
    ...
    padding: '12px 20px', borderRadius: 12, backdropFilter: 'blur(12px)',
...
const MakeAdmin = () => {
    ...
    const volunteerGet = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/home/admin-dashboard/make-admin`,
  ...
  const approve = async (elem) => {
    try {
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/home/admin-dashboard/make-admin/approved-to-admin`,
        { name: elem.name, email: elem.email, aprove: 'aproved', latitude, longitude },
        { headers: { 'Content-Type': 'application/json', Authorization: token.token } }
      )
      // FIXED: Changed setData to setVolunteerData
      setVolunteerData(prev => prev.filter(d => d._id !== elem._id))
      showToast(`${elem.name} approved to admin successfully`, 'success')
    } catch (e) {
      console.error(e)
    ...
    <div className="min-h-screen bg-gray-950 px-8 py-11 relative overflow-hidden">
      ...
      <div className="relative z-10 mb-9">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <p className="text-xs font-mono tracking-[0.22em] uppercase text-sky-400 mb-1.5">Admin Dashboard</p>
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {loading
          ...
          : filtered.length > 0
            ...
            : (
              <div className="col-span-full text-center py-16 text-slate-700 font-mono text-sm">No volunteers found.</div>
            )
        }
      </div>
